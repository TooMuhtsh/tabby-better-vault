...
| `registre-livrés` | `oui` | tranché au cadrage du 2026-08-06 : la roadmap dépasse 1700 lignes, en grande partie des chantiers déjà clos — exactement le cas que l'option cible. **Décision de principe seulement** : la migration réelle des chantiers livrés vers `annexes/REALISE.html` est un chantier à part, consigné en roadmap, pas encore fait |
| `journal-format` | **Écart assumé à l'option de la charte** — pas `tableau` ni `log` (qui restent tous deux du HTML), mais un vrai fichier `AI-HISTORY.log` en texte brut, colonnes `horodatage \| CHANTIER \| Commit/Hash \| Explication` (Explication ≤ 250 caractères), une ligne par entrée d'historique | demandé explicitement le 2026-08-06 après clarification (l'option `journal-format` de la charte ne couvre que la structure du tableau, pas le format de fichier). Rompt l'uniformité `format = html` sur les quatre documents, **pour `AI-HISTORY` seulement** — décision de principe actée ici, la conversion elle-même (migration des ~900 lignes existantes, liens de navbar, `lint:airules` qui ne valide que du HTML) est un chantier à part, consigné en roadmap, pas encore fait |
